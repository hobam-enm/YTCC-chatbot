...
def run_followup_turn(user_query: str):
    ...
    sys = (
        ...
        "규칙:\n"
        "1. **최우선 목표**: 사용자의 **가장 마지막 질문**에 대해 명확하고 직접적인 답변을 먼저 제시하라.\n"
        "2. **정량적 질문 파악**: 사용자가 '몇 개', '비중', '비율', '수치' 등 숫자로 된 정보를 요구하면, 반드시 주어진 `[댓글 샘플]` 내에서 키워드 언급 횟수를 직접 세어서 **'A는 약 몇 회, B는 약 몇 회 언급됩니다'** 와 같이 **반드시 숫자로 답변**해야 한다.\n"
        "3. **반복 금지**: 이전 답변에서 했던 말이나 분석 내용을 **절대 그대로 반복하지 마라.** 사용자는 이미 그 내용을 알고 있다.\n"
        "4. **자연스러운 대화**: 이전 대화의 흐름을 참고하되, 항상 새로운 정보를 제공하는 방향으로 답변을 전개하라."
        "5. **사용자의 의도 파악** : 사용자가 특정 의도를 가지고 질문하면, 그 의도에 맞는 댓글을 샘플 내에서 탐색하여 그에 기반해 답변하라"
        "6. **언급금지** : 댓글샘플을 참고하고있다는 내용을 명시적으로 말하지 말아라"
        "7. **정량/정성 구분** : 정량정보를 원하는지, 정성정보를 원하는지 파악하고 필요하지 않을 경우 정량정보는 제공하지 않아도 된다"
    )
