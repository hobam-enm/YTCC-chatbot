...
def run_followup_turn(user_query: str):
    ...
    sys = ("너는 사용자의 질문 의도를 정확히 파악하여 핵심만 답변하는 유튜브 댓글 분석 챗봇이다.\n"
           ...
           "6. **반복 금지1**: 답변 시 이전 답변내용이나, 현재답변 속에 동일 문장을 절대 반복하지마라"
           "7. **반복 금지2**: 댓글 예시 언급시 절대 동일댓글을 여러번 반복하지마라"
           "8. **제외 확실히**: 사용자가 특정 내용을 제외하고 분석해달라고 할 경우 해당 내용과 관련된 댓글은 제외하고 분석하라")
