# -*- coding: utf-8 -*-
# 💬 유튜브 댓글분석기 — 챗봇 모드 (심플 실행 + 진행바 + AI요약 노출 보장)
...
class RotatingYouTube:
    ...
    def execute(self, request_factory):
        try:
            ...
            if status in (403,429) and any(x in msg for x in ["quota","rate","limit"]) and len(YT_API_KEYS)>1:
                self.rot.rotate()
                self.service = build("youtube","v3",developerKey=self.rot.current())
...
def keyword_bubble(df_counts: pd.DataFrame):
    if df_counts is None or df_counts.empty: return None
    df = df_counts.copy()
    df["label"] = df["word"] + "<br>" + df["count"].astype(str)
    df["scaled"] = np.sqrt(df["count"])
    circles = circlify.circlify([{"id": w, "datum": s} for w, s in zip(df["word"], df["scaled"])],
                                show_enclosure=False, target_enclosure=circlify.Circle(x=0, y=0, r=1))
    pos = {c.ex["id"]:(c.x,c.y,c.r) for c in circles if "id" in c.ex}
    df["x"] = df["word"].map(lambda w: pos[w][0])
    df["y"] = df["word"].map(lambda w: pos[w][1])
    df["r"] = df["word"].map(lambda w: pos[w][2])
    s_min, s_max = df["scaled"].min(), df["scaled"].max()
    df["font_size"] = df["scaled"].apply(lambda s: int(10 + (s - s_min)/max(s_max - s_min, 1) * 12))
    fig = go.Figure()
    palette = px.colors.sequential.Blues
    df["color_idx"] = df["scaled"].apply(lambda s: int((s - s_min)/max(s_max - s_min, 1) * (len(palette) - 1)))
    ...
    fig.add_trace(go.Scatter(x=df["x"], y=df["y"], mode="text",
                             text=df["label"], textposition="middle center",
                             textfont=dict(color="white", size=df["font_size"].tolist())))
...
if run and user_query:
    ...
    if not GEMINI_API_KEYS:
        st.error("Gemini API Key가 없습니다. st.secrets에 GEMINI_API_KEYS를 설정하세요."); st.stop()
    ...
    # 2) 영상 검색/통계
    if not YT_API_KEYS:
        st.error("YouTube API Key가 없습니다. st.secrets에 YT_API_KEYS를 설정하세요."); st.stop()
    ...
    # ===== 결과 노출 (배지 + 요약) =====
    st.markdown(
        f"<div style='font-size:12px;color:#6b7280;margin-top:6px;margin-bottom:8px;'>"
